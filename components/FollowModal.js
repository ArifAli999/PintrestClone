  ...
  return (
    ...
    <>
          ...
          <div className=' border border-gray-300 px-4 p-2 rounded-full hover:border-pink-500 transition-all ease-in-out duration-300 cursor-pointer' onClick={openModal}>
              ...
              <span className='text-pink-500 font-semibold  text-base flex items-center gap-2'>{followers.FollowedBy?.length === 0 ? null : followers.FollowedBy?.length} <span className='text-gray-400 text-xs'>{followers && followers.FollowedBy?.length > 1 ? 'Followers' : 'Follower'}</span> </span>
